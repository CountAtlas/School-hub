...
type ApprovedItem = {
  ...
  subject: string;
  classLevel: "11" | "12";
  section: "notes" | "practicals" | "resources";
  author?: string;
  description?: string;
  fileUrl: string;
...
export default async function HomePage() {
  const submissions = (await readSubmissions()) as ApprovedItem[];
  ...
  const notesCount = approved.filter(
    (item) => item.section === "notes"
  ).length;

  const practicalsCount = approved.filter(
    (item) => item.section === "practicals"
  ).length;

  const resourcesCount = approved.filter(
    ...
    <main className="min-h-screen text-white">
      <section className="relative overflow-hidden px-6 py-20">
        ...
        <div className="relative mx-auto max-w-6xl">
          ...
          <p className="mt-6 max-w-2xl text-lg text-zinc-400">
            Notes, practicals, projects, and study resources collected by
      ...
      <section className="px-6 py-16">
        <div className="mx-auto grid max-w-6xl grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur-md">
            <h2 className="text-3xl font-bold">{subjects.length}</h2>
        ...
        <div className="mx-auto mt-6 grid max-w-6xl gap-6 md:grid-cols-3">
          <div className="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur-md">
            <h2 className="text-3xl font-bold">{notesCount}</h2>
            <p className="mt-1 text-sm text-zinc-400">Notes</p>
          </div>

          <div className="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur-md">
            <h2 className="text-3xl font-bold">{practicalsCount}</h2>
            <p className="mt-1 text-sm text-zinc-400">Practicals</p>
          </div>

          <div className="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur-md">
            <h2 className="text-3xl font-bold">{resourcesCount}</h2>
        ...
        <div className="mx-auto max-w-6xl">
          <div className="mb-10">
            ...
            <p className="mt-3 text-zinc-400">
              Approved study material from the community.
            </p>
          </div>
      ...
      <section className="px-6 pb-20">
        <div className="mx-auto max-w-4xl rounded-3xl border border-violet-500/20 bg-violet-500/5 p-8 text-center backdrop-blur-md">
          ...
          <p className="mt-4 text-zinc-400">
            Help other students learn faster by contributing your notes.
          </p>
